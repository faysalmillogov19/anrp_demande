...
def test(request):
	...
	produits= [
				['Libellé', 'Quantité', 'Prix', 'Dévise'],
       			['Avici djdjd jdjdjdjdbd dhdjdjd jdjdjd jdjdjd ', '11', '20','Euro'],
       			['Avici djdjd jdjdjdjdbd dhdjdjd jdjdjd jdjdjd ', '11', '20','Euro'],
       			['Avici djdjd jdjdjdjdbd dhdjdjd jdjdjd jdjdjd ', '11', '20','Euro'],
    ]
	filename=add_text_Recepice_with_table(data,produits,qr_code,)
...
def add_text_Recepice_with_table(input, table_list, img):
	file_name='static/uploads/Recepice/'+str(datetime.now().strftime("_%Y_%m_%d_%H_%M_%S"))+'.pdf'
	packet = io.BytesIO()
	can = canvas.Canvas(packet, pagesize=letter)
	can.setFillColorRGB(0, 0, 0)
	...
	x = 100
	y = 800
	t = Table(table_list, width, height)
