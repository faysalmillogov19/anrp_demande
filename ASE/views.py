...
def add_produit(request, id_demande):
	...
	if request.method=="POST":
		...
		return render(request,'ASE/add_produit.html',{'categories':categories,'id_demande':id_demande,'devises':devises})
...
def set_produit(request, id):
	if request.method=='POST':
		...
		categories=Categorie.objects.all()
		return render(request,'ASE/set_produit.html',{'categories':categories,'produit':produit,'devises':devises})
...
def expediteur(request, id_demande):
	if request.method=="POST":
		...
		element=ASE.objects.get(id=id_demande)
		return render(request,'ASE/add_expediteur.html',{'element':element,'id_demande':id_demande,'type_expediteurs':type_expediteurs,'voies':voies,'objets':objets})
...
def pieces_jointes(request, id_demande):
	if request.method=="POST":
		...
		if request.FILES.get("copie_asi"):
			element.copie_asi=request.FILES.get("copie_asi")
...
def valid_demande(request, id_demande):
	if request.method=='POST':
		...
		qr_code=generateQRCode(info.url_recepice+'ase/'+str(asee.id), 'static/uploads/Recepice/')
