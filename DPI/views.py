...
def add_produit(request, id_demande):
	...
	if request.method=="POST":
		...
		element.code_douanier=request.POST.get('code_douanier')
		element.save()
		return redirect('dpi_list_produit', id_demande=element.demande.id)
	else:
		devises=Devise.objects.all()
		categories=Categorie.objects.all()
		return render(request,'DPI/add_produit.html',{'categories':categories,'id_demande':id_demande,'devises':devises})
...
def set_produit(request, id):
	if request.method=='POST':
		...
		element.cout=request.POST.get('cout')
		element.code_douanier=request.POST.get('code_douanier')
		print(request.POST.get('code_douanier'))
		element.save()
		return redirect('dpi_list_produit', id_demande=element.demande.id)
	else:
		devises=Devise.objects.all()
		...
		categories=Categorie.objects.all()
		return render(request,'DPI/set_produit.html',{'categories':categories,'produit':produit,'devises':devises})
...
def expediteur(request, id_demande):
	if request.method=="POST":
		...
		element=DPI.objects.get(id=id_demande)
		return render(request,'DPI/add_expediteur.html',{'element':element,'id_demande':id_demande,'type_expediteurs':type_expediteurs,'voies':voies,'objets':objets})
...
def pieces_jointes(request, id_demande):
	if request.method=="POST":
		...
		if request.FILES.get("document_douane"):
			element.document_douane=request.FILES.get("document_douane")
...
def valid_demande(request, id_demande):
	if request.method=='POST':
		...
		qr_code=generateQRCode(info.url_recepice+'dpi/'+str(dpii.id), 'static/uploads/Recepice/')
