...
def list_produit(request, id):
	...
	#commentaires=Traitement.objects.get(demande_id=id)
	produits=Produit_demande.objects.filter(demande=Demande.objects.get(id=id))
	produit_amm=Produit.objects.all()
	return render(request,'Espace_client/ase/list_produit.html',{'datum':datum,'produits':produits,'statut':statut,'categories':categories,'produit_amm':produit_amm,"can_treat":can_treat,"can_print":can_print,'natures':natures,'commentaires':'commentaires'})
...
def treat(request, id):
	if request.method=="POST":
		...
		ase.statut=statut
		ase.note=request.POST.get('note')
	...
	if request.POST.get('commentaire'):
		...
		traitement.utilisateur=request.user.first_name
		traitement.save()
...
def print_ase(request, id):
	...
	libelle_demande="l'exportation"
	intitule_demande="AUTORISATION SPECIALE D'EXPORTATION"
	infos=Infos.objects.get(id=1)
	message=EmailMessage.objects.get(id=2)

	if datum.nature_impression_id == 1:
		return render(request, "Espace_client/ase/ase_print.html", {'datum':datum,'produits':produits,'signataire':signataire, 'libelle_demande':libelle_demande,'intitule_demande':intitule_demande,'infos':infos,'message':message} )
